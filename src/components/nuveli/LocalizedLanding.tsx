      ...
      <header className="border-b border-border">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5 md:px-10">
          ...
          <nav aria-label={t.language} className="flex gap-2 text-xs">
            <a href="/" hrefLang="sl" className="rounded-full border border-border px-3 py-2">SL</a>
            <a href="/hr/" hrefLang="hr" className={`rounded-full border px-3 py-2 ${locale === "hr" ? "border-foreground" : "border-border"}`}>HR</a>
            <a href="/en/" hrefLang="en" className={`rounded-full border px-3 py-2 ${locale === "en" ? "border-foreground" : "border-border"}`}>EN</a>
          </nav>
        </div>
      </header>
      ...
      <main>
        <section className="mx-auto max-w-6xl px-6 py-20 md:px-10 md:py-28">
          ...
          <div className="mt-10 flex flex-col items-start gap-3">
            <a href={`${prefix}/home-dna`} className="inline-flex min-h-12 items-center rounded-full bg-primary px-7 py-4 text-sm text-primary-foreground">
              {t.cta}
            </a>
          ...
          <div className="mx-auto max-w-6xl px-6 py-16 md:px-10 md:py-20">
            ...
            <div className="mt-10 grid gap-x-10 gap-y-0 md:grid-cols-2">
              {aiSearchArticles.map((article, index) => {
                ...
                return (
                  <a key={article.id} href={articlePath(article, locale)} className="group border-t border-border py-6">
                    <span className="eyebrow">{String(index + 1).padStart(2, "0")}</span>
                    <h3 className="mt-3 font-display text-xl group-hover:underline">{content.title}</h3>
