...
export function RoomSelection({
  ...
  const copy = locale === "hr"
    ? { headline: "Koje prostore želite urediti?", support: "Odaberite jedan prostor, više prostora ili cijeli dom. Discovery ćemo automatski prilagoditi opsegu vašeg projekta.", aria: "Odabir prostora" }
    : locale === "en"
      ? { headline: "Which spaces would you like to design?", support: "Choose one room, several rooms or the whole home. Discovery will automatically adapt to your project scope.", aria: "Room selection" }
      : { headline: "Katere prostore želite urediti?", support: "Izberite en prostor, več prostorov ali celoten dom. Discovery bomo samodejno prilagodili obsegu vašega projekta.", aria: "Izbira prostorov" };
  ...
  return (
    <div>
      ...
      <p className="mt-6 max-w-[58ch] text-base leading-relaxed text-muted-foreground">{copy.support}</p>
      <div role="group" aria-label={copy.aria} className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {roomOptions.map((room) => {
          ...
          return (
            <VisualChoiceCard
              ...
              description={isChildrenRoom && quantity ? childrenDescription(quantity) : uiText(locale, room.description) ?? room.description}
              multiSelect
      ...
      <DiscoveryNavigation locale={locale} onBack={onBack} onNext={onNext} showNext={selectedRooms.length > 0} nextDisabled={selectedRooms.length === 0} />
    </div>
  );
}
