...
export function NoteScreen({
  ...
  const labels =
    locale === "hr"
      ? {
          label: "Vaša napomena",
          placeholder: "Ukratko opišite što biste željeli poboljšati.",
          next: "Dalje",
          skip: "Preskoči",
        }
      : locale === "en"
        ? {
            label: "Your note",
            placeholder: "Briefly describe what you would like to improve.",
            next: "Next",
            skip: "Skip",
          }
        : {
  ...
  return (
    <ScreenShell screenKey={screenKey} headline={headline} support={support}>
      ...
      <DiscoveryNavigation
        ...
        nextLabel={text.trim() ? labels.next : labels.skip}
      />
    </ScreenShell>
  );
}
