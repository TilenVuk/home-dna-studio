...
const ImageChoiceInput = z.object({
  ...
  matchReasons: z.array(z.string().trim().min(1).max(100)).max(6).optional(),
});
...
export const generateHomeDnaReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (origin && !isAllowedRequestOrigin(request, origin)) throw new Error("HOME_DNA_INVALID_ORIGIN");
